...
export async function runDailyReport() {
  console.log('⏰ Executing ApexVoice Daily Report Runner...')
...
Status: 24/7 ACTIVE | echoliftai.co.uk`
  ...
  // 3. Send SMS via Twilio using Alphanumeric Sender ID 'EchoLift'
  ...
  if (sid && token) {
    try {
      ...
      if (res.ok) {
        smsStatus = 'Delivered via Twilio (Sender ID: EchoLift)'
        console.log('✅ Daily Report SMS sent successfully!')
      } else {
        smsStatus = `Failed (${res.status}): ${resText.slice(0, 200)}`
        console.error('❌ Daily Report SMS failed:', res.status, resText)
      }
    } catch (err: any) {
    ...
    smsStatus = 'Failed: Missing Twilio credentials'
  }
  ...
  try {
    const html = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; background: #0f172a; color: #f8fafc; padding: 24px; border-radius: 12px;">
        ...
        <p style="color: #94a3b8; font-size: 14px;">Date: ${todayStr} ${timeStr}</p>
